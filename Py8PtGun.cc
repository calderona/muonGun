...
bool Py8PtGun::generatePartonsAndHadronize()
{
   ...
   for ( size_t i=0; i<fPartIDs.size(); i++ ){
      ...
      double p   = (fMaxP-fMinP) * randomEngine().flat() + fMinP;
      
      double mass = (fMasterGen->particleData).m0( particleID );

      //double pp = p / sin(the); // sqrt( ee*ee - mass*mass );
      double ee = sqrt( p*p + mass*mass );
      
      double px = p * sin(the) * cos(phi);
      double py = p * sin(the) * sin(phi);
      double pz = p * cos(the);
